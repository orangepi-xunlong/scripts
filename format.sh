...
if [ ! "${sdcard}" = "" ]; then
    ...
    dd if=$TOP/external/Legacy_patch/orange/boot0_OPI.fex of=${sdcard} bs=1k seek=8 ${_ddparm} > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "ERROR installing boot0."
        exit 1
    fi
    dd if=$TOP/external/Legacy_patch/orange/u-boot_OPI.fex of=${sdcard} bs=1k seek=16400 ${_ddparm} > /dev/null 2>&1
    if [ $? -ne 0 ]; then
